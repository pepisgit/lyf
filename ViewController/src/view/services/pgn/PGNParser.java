...
public class PGNParser 
{
                            ...
                            try 
                            {
                                ...
                                try
                                {
                                    if (tagName.equals("Event")){
                                        game.setBpgn_event(tagValue);
                                    }
                                    else if (tagName.equals("Result")){
                                        game.setBpgn_result(tagValue);
                                    }
                                    else if (tagName.equals("ECO")){
                                        game.setBpgn_eco(tagValue);
                                    }
                                    else if (tagName.equals("Round")){
                                        game.setBpgn_round(Integer.valueOf(tagValue));
                                    }
                                    else if (tagName.equals("White")){
                                        game.setBpgn_white(tagValue);
                                    }
                                    else if (tagName.equals("Black")){
                                        game.setBpgn_black(tagValue);
                                    }
                                    else if (tagName.equals("Date")){
                                        // TODO Parsear fecha
                                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd");
                                        game.setBpgn_date(sdf.parse(tagValue));
                                    }
                                }
                                catch (Exception ee){
                                    //ee.printStackTrace();
    ...
    private static void handleRawMoves(String[] rawMoves, PGNGame game, byte[][] board, int[] color) throws MalformedMoveException, PGNParseException, NullPointerException {
            ...
            for (int i = 0; i < rawMoves.length; i++) {
                    ...
                    } else if (rawMoves[i].startsWith("$")) {
                            move.setAssessment(rawMoves[i]);
                    } else {
